...
class Bottleneck_Residual(nn.Module):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(Bottleneck_Residual, self).__init__()
...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv3d(inplanes, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm3d(planes)
        self.conv2 = nn.Conv3d(planes // beta,
                               planes // alpha * (alpha - 1),
                               kernel_size=(1, 3, 3),
                               stride=(1, stride, stride),
                               padding=(0, 1, 1),
                               bias=False)
        self.Tconv = nn.Conv3d(planes // beta,
                               planes // alpha,
                               kernel_size=3,
                               bias=False,
                               stride=(1, stride, stride),
...
class ResNet(nn.Module):
    def __init__(self,
        ...
        self.resi = resi
        if resi:
            self.beta_resi = 1 / 8.0
            self.residual_inplanes = int(512 * self.beta_resi)
            self.lateral_layer3 = self._make_normal_layer(
                Bottleneck_Residual, int(128 * self.beta_resi), 1)
            self.backward_layer3 = self._make_normal_layer(
                Bottleneck_Residual, int(128 * self.beta_resi), 1)
            self.residual_inplanes = int(1024 * self.beta_resi)
            self.lateral_layer4 = self._make_normal_layer(
                Bottleneck_Residual, int(256 * self.beta_resi), 1)
            self.backward_layer4 = self._make_normal_layer(
                Bottleneck_Residual, int(256 * self.beta_resi), 1)
    ...
    def _make_normal_layer(self, block, planes, blocks, stride=1):
        downsample = None
        if stride != 1 or self.residual_inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv3d(self.residual_inplanes,
                          planes * block.expansion,
                          kernel_size=1,
                          stride=(1, stride, stride),
                          bias=False),
        ...
        layers = []
        layers.append(block(self.residual_inplanes, planes, stride,
                            downsample))
        self.residual_inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.residual_inplanes, planes))
...
def resnet50(alpha, beta, **kwargs):
    ...
    #print(checkpoint['layer1.0.conv1.weight'][:20])
    for ln in layer_name:
        if not 'lateral' in layer_name:
            if 'conv' in ln or 'downsample.0.weight' in ln:
                checkpoint[ln] = checkpoint[ln].unsqueeze(2)
            if 'conv2' in ln:
                n_out, n_in, _, _, _ = checkpoint[ln].size()
                checkpoint[ln] = checkpoint[ln][:n_out // alpha *
                                                (alpha - 1), :n_in //
                                                beta, :, :, :]
    model.load_state_dict(checkpoint, strict=False)
    #print(model.state_dict()['layer1.0.conv1.weight'][:20])
    return model
