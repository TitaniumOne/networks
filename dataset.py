...
class VideoDataset(data.Dataset):
    ...
    def _load_image(self, directory, idx):
        ...
        image_path = Path(self.root_path + '/' + directory + '/' + self.image_tmpl.format(idx))
        ...
        if self.modality == 'RGB' or self.modality == 'RGBDiff':
            ...
            x_img = Image.open(Path(self.root_path + '/' + directory + '/' + self.flow_tmpl.format('x', idx))).convert('L')
            y_img = Image.open(Path(self.root_path + '/' + directory + '/' + self.flow_tmpl.format('y', idx))).convert('L')
    ...
    def _sample_indices(self, record):
        ...
        average_duration = (num_frames - self.new_length +
                            1) // self.num_segments
        if average_duration > 0:
            offsets = np.multiply(list(range(self.num_segments)),
                                  average_duration) + randint(
                                      average_duration, size=self.num_segments)
        elif num_frames - self.new_length + 1 > self.num_segments:
            offsets = np.sort(
                randint(num_frames - self.new_length + 1,
    ...
    def _get_val_indices(self, record):
        ...
        if num_frames > self.num_segments + self.new_length - 1:
            tick = (num_frames - self.new_length + 1) / float(
                self.num_segments)
            offsets = np.array(
    ...
    def get(self, record, indices):
        ...
        if self.num_clips > 1:
            ...
            process_data, label = self.transform((images, record.label))
            # print("process_data.size = {}".format(process_data.size()))
